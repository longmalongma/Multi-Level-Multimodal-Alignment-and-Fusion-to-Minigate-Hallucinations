...
from lavis.models.blip2_models.Qformer_clip import BertConfig, BertLMHeadModel, BertSelfAttention, BertAttention, BertLayer, BertModel, BertEncoder
...
class Blip2Base(BaseModel):
    ...
    @classmethod
    def init_Qformer(cls, num_query_token, vision_width, cross_attention_freq=2, memory_bank_length=0, num_frames=0,clip4qformer = None):
        ...
        Qformer = BertLMHeadModel.from_pretrained(
            "bert-base-uncased", config=encoder_config,clip4qformer = clip4qformer,
        )
...
def memory_bank_compress_cm(memory_bank: torch.Tensor, compression_size: torch.Tensor, reference_t) -> tuple:
    """
    Compresses the memory bank if the current memory bank length is greater than the threshold.
    Compression_size is the number of frames that are compressed into each position.
    ...
    """
    # ԭʼ�汾
    B, T, N, C = memory_bank.shape # [B, t+1, 257, 1408]
    original_memory = memory_bank[:,:-1] # [B, t, 257, 1408]
    last_memory = memory_bank[:,-1]  # [B, 257, 1408]
    last_memory = last_memory.unsqueeze(1).repeat(1, T-1, 1, 1) # [B, t, 257, 1408]
    
    # ������֡���֮֡����������ƶȣ�������һά����������������֮֡������ƶȣ���ʱû��ͨ���ĸ�����
    similarity_matrix = F.cosine_similarity(original_memory[:, :, :], last_memory[:, :, :], dim=-1) # [B, t, 257]
    # ���������Ƶ�����������ÿ�е����ֵ�����һ���¼����������ÿ�����ֵ���ڵ�λ��
    max_similarity, max_similarity_indices = torch.max(similarity_matrix, dim=1, keepdim=True) # [B, 1, 257]

    for b in range(B):
        similarity_b = similarity_matrix[b]  # [t, 257] ����֡���ԭʼ֡��ÿ��patch�ϵ����ƶ�
        max_similarity_b = max_similarity[b] # [1, 257]    ����֡����patch��ԭʼ֡��Ӧpatch������ƶ�
        max_similarity_indices_b = max_similarity_indices[b][0] # [257] ����֡����patch��ԭʼ֡��Ӧpatch������ƶȵ�ԭʼ֡����
        # �����֡���ƶ�����ԭʼ֡���ƶ�
        max_similarity_bb, max_similarity_bbi = torch.max(max_similarity_b, dim=1, keepdim=False) 
        # �����֡���ƶ�����ԭʼ֡����
        max_similarity_indices_bb = max_similarity_indices_b[max_similarity_bbi.item()]
        if max_similarity_bb.item() > 0.95:
            original_memory[b][max_similarity_indices_bb] = 0.1 * original_memory[b][max_similarity_indices_bb] + 0.9 * last_memory[b][0]
            reference_t[b][max_similarity_indices_bb] += 1
        else:
            min_times, min_times_indices = torch.min(reference_t[b].unsqueeze(0), dim=1, keepdim=False)
            original_memory[b][min_times_indices.item():T-2] = original_memory[b][min_times_indices.item()+1:T-1].clone()
            original_memory[b][-1] = last_memory[b][0]
            reference_t[b][min_times_indices.item():T-2] = reference_t[b][min_times_indices.item()+1:T-1].clone()
            reference_t[b][-1] = 0
            
    return original_memory, compression_size, reference_t
