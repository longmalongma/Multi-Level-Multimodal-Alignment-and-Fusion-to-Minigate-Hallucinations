...
class Blip2VicunaInstruct_MALMM(Blip2Base):
    ...
    def __init__(
        ...
        self.llm_proj_in_dim = self.Qformer.config.hidden_size + self.T1.aim_hidden_size
        self.llm_proj = nn.Linear(
            self.llm_proj_in_dim, self.llm_model.config.hidden_size
        )
    ...
    def forward(self, samples):
        ...
        # 先对齐特征，再与原始输出在通道维拼接
        aligned_qformer = self.T1.align_qformer_features(
            query_output.last_hidden_state[:, :query_tokens.size(1), :]
        )
        aligned_qformer = aligned_qformer.unsqueeze(1).expand(-1, query_tokens.size(1), -1)
        fused_qformer_output = torch.cat(
            [query_output.last_hidden_state[:, :query_tokens.size(1), :], aligned_qformer],
            dim=-1,
        )
    ...
    def load_state_dict(self, state_dict, strict=False):
        """
        兼容旧 checkpoint：llm_proj 输入维度由 768 扩展到 1536 时，迁移旧权重并对新增通道重新初始化。
        """
        key = "llm_proj.weight"
        if key in state_dict:
            pretrained_w = state_dict[key]
            target_w = self.llm_proj.weight
            if pretrained_w.shape != target_w.shape:
                new_w = target_w.clone()
                copy_dim = min(pretrained_w.shape[1], new_w.shape[1])
                new_w[:, :copy_dim] = pretrained_w[:, :copy_dim]
                if copy_dim < new_w.shape[1]:
                    nn.init.xavier_uniform_(new_w[:, copy_dim:])
