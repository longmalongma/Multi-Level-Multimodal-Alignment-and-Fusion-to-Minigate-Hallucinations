...
class T1(nn.Module):
    ...
    def align_qformer_features(self, qformer_features):
        """
        对齐 qformer 特征，与 T1 forward 中的处理方式保持一致。
        
        Args:
            qformer_features: [B, N, C] 形状的 qformer 输出特征
        
        Returns:
            aligned_features: [B, aim_hidden_size] 对齐后的特征
        """
        if self.way == "linear":
