...
@registry.register_model("blip2_vicuna_instruct_first_malmm")
class Blip2VicunaInstruct_MALMM(Blip2Base):
    ...
    def __init__(
        ...
        self.clip_tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        self.clip_text_embedding = BertEmbeddings(config=self.clip_embed_config )
        self.clip_max_length = int(max_txt_len+0.5*max_output_txt_len+1)
        self.T1 = clip4qformer.T1(self.clip_tokenizer,
                                            self.clip_text_embedding,
                                            config=self.clip_embed_config ,
                                            way="avgpool",
        ...
        self.Qformer, self.query_tokens = self.init_Qformer(
            ...
            clip4qformer=clip4qformer.T1(self.clip_tokenizer,
                                            self.clip_text_embedding,
                                            config=self.clip_embed_config ,
                                            way="avgpool",
                                            max_length=self.clip_max_length),
    ...
    def forward(self, samples):
        ...
        loss =  0.5 * clip_loss/(clip_counter+1e-8 ) + 0.5 * clip_cross_loss_sum/(clip_counter+1e-8 )
