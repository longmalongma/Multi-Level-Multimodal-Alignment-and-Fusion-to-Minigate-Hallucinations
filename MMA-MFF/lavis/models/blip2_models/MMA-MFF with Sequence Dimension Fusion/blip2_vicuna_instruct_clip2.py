...
class Blip2VicunaInstruct_MALMM(Blip2Base):
    ...
    def __init__(
        ...
        self.align_gate = nn.Sequential(
            nn.Linear(self.Qformer.config.hidden_size, self.Qformer.config.hidden_size // 4),
            nn.ReLU(),
            nn.Linear(self.Qformer.config.hidden_size // 4, self.Qformer.config.hidden_size),
            nn.Sigmoid()
        )
        # 残差投影，用于增强特征融合
        self.residual_proj = nn.Linear(self.Qformer.config.hidden_size, self.Qformer.config.hidden_size)
    ...
    def load_state_dict(self, state_dict, strict=False):
        """
        兼容旧checkpoint：新添加的align_proj模块在旧权重中不存在，
    ...
    def fuse_features_in_sequence(self, qformer_output, query_tokens_size):
        """
        改进的序列维度对齐：使用门控机制和残差连接增强特征融合
        ...
        Returns:
            fused_output: 融合后的特征，形状为 [B, query_tokens_size+1, C]
        """
        ...
        # 将对齐特征投影到QFormer的hidden_size [B, hidden_size]
        aligned_proj = self.align_proj(aligned_features)
        
        # 改进1：使用门控机制自适应控制对齐特征
        # 计算query tokens的平均特征作为上下文
        query_context = qformer_query_output.mean(dim=1)  # [B, C]
        # 门控信号
        gate = self.align_gate(query_context)  # [B, C]
        # 应用门控
        gated_aligned = aligned_proj * gate
        
        # 改进2：添加残差连接增强对齐特征
        residual_aligned = self.residual_proj(gated_aligned)
        enhanced_aligned = gated_aligned + residual_aligned
        
        # 改进3：使用可学习权重缩放对齐特征（保持对齐特征的独立性）
        # 使用sigmoid确保权重在合理范围内
        fusion_scale = torch.sigmoid(self.fusion_weight)
        scaled_aligned = enhanced_aligned * fusion_scale
        
        # 扩展为序列维度 [B, 1, hidden_size]
        aligned_proj_seq = scaled_aligned.unsqueeze(1)
        
        # 改进4：将对齐特征放在序列末尾（更有利于分类任务，作为总结性特征）
        # 在序列维度拼接：将对齐特征添加到序列的末尾
        fused_output = torch.cat([qformer_query_output, aligned_proj_seq], dim=1)  # [B, query_tokens_size+1, C]
    ...
    def forward(self, samples):
        ...
        # 在序列维度对齐：将对齐特征添加到序列中
        fused_qformer_output = self.fuse_features_in_sequence(
    ...
    def _predict_class(
        ...
        if image.dim() == 5:
            ...
            for j in range(image.size(2)):
                ...
                # 在序列维度对齐：将对齐特征添加到序列中
                fused_frame_output = self.fuse_features_in_sequence(
