...
class Blip2VicunaInstruct_MALMM(Blip2Base):
    ...
    def forward(self, samples):
        ...
        if image.dim() == 5:
            ...
            print("max_epoch:", samples["max_epoch"])
            print("num_iters_per_epoch:", samples["num_iters_per_epoch"])
            print("cur_epoch:",samples["epoch"])
            print("cur_iter:", samples["iters"])
            print(f"T:",T)

            used_frame_rate = ((samples["epoch"]+1)*samples["num_iters_per_epoch"] + samples["iters"]) \
             / (samples["max_epoch"]*samples["num_iters_per_epoch"]) #+ 0.5

            used_frames_num = round(used_frame_rate * T)

            if used_frames_num < 10: 
                used_frames_num = 10
            elif used_frames_num > T:
                used_frames_num = T
            
            indices = torch.linspace(0, T - 1, used_frames_num).long()
            sampled_frames = image[:, :, indices, :, :] 

            image = sampled_frames
            T = used_frames_num
            self.T = T

            for module in self.Qformer.bert.modules():
                if module.__class__ == MBBertSelfAttention:
                    module.num_frames = T
    ...
    def generate(
        ...
        if image.dim() == 5:
            ...
            if self.T != T:
                indices = torch.linspace(0, T - 1, self.T).long()
                sampled_frames = image[:, :, indices, :, :] 
