...
class RunnerBase:
    ...
    def resume_ckpt_path(self):
        ckpt_path = self.config.run_cfg.get("resume_ckpt_path", None)
        if ckpt_path == None:
            if self.evaluate_only:
                return os.path.join(self.output_dir, 'checkpoint_best.pth')
            else:
                return os.path.join(self.output_dir, 'checkpoint_latest.pth')
        else:
            return ckpt_path
    ...
    def setup_output_dir(self):
        ...
        result_dir = output_dir / "result" / f"beam{self.config.run_cfg.num_beams}"
    ...
    def train(self):
        ...
        for cur_epoch in range(self.start_epoch, self.max_epoch):
            ...
            if not self.evaluate_only:
                ...
                train_stats = self.train_epoch(cur_epoch, self.max_epoch)
                ...
                for i, split_name in enumerate(self.valid_splits):
                    ...
                    if val_log is not None:
                        if is_main_process():
                            ...
                            if agg_metrics > best_agg_metric[i]:
                                best_agg_metric[i] = agg_metrics
                                for key, value in val_log.items():
                                    best_val_log[i][key + '_best'] = value

                                if not self.evaluate_only and split_name == "val":
    ...
    def _save_checkpoint(self, cur_epoch, is_best=False):
        ...
        save_to = os.path.join(
            self.output_dir,
            "checkpoint_{}.pth".format("best" if is_best else 'latest'),
        )
    ...
    def _load_checkpoint(self, url_or_filename):
        ...
        self.start_epoch = checkpoint["epoch"] + 1 if not self.evaluate_only else checkpoint["epoch"]
